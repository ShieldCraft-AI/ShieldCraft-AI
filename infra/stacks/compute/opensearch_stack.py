...
class OpenSearchStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        node_to_node_encryption_options = domain_cfg.get(
            "node_to_node_encryption_options", {"enabled": True}
        )
        encryption_at_rest_options = domain_cfg.get(
            "encryption_at_rest_options", {"enabled": True}
        )
        ebs_options = domain_cfg.get(
            "ebs_options", {"ebsEnabled": True, "volumeSize": 20, "volumeType": "gp3"}
        )
        cluster_config = domain_cfg.get(
            "cluster_config", {"instanceType": "m6g.large.search", "instanceCount": 2}
        )
        advanced_security_options = domain_cfg.get(
