...
class MskStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, vpc: ec2.IVpc, config: dict, **kwargs):
        ...
        self.broker_count_alarm = cloudwatch.Alarm(
            self, f"{construct_id}MskBrokerCountAlarm",
        ...
        self.under_replicated_alarm = cloudwatch.Alarm(
            self, f"{construct_id}MskUnderReplicatedPartitionsAlarm",
            metric=cloudwatch.Metric(
                namespace="AWS/Kafka",
        ...
        self.active_controller_alarm = cloudwatch.Alarm(
            self, f"{construct_id}MskActiveControllerCountAlarm",
            metric=cloudwatch.Metric(
                namespace="AWS/Kafka",
        ...
        self.disk_used_alarm = cloudwatch.Alarm(
            self, f"{construct_id}MskDiskUsedAlarm",
            metric=cloudwatch.Metric(
                namespace="AWS/Kafka",
        ...
        # Outputs for alarms
        CfnOutput(self, f"{construct_id}MskBrokerCountAlarmArn", value=self.broker_count_alarm.alarm_arn, export_name=f"{construct_id}-msk-broker-count-alarm-arn")
        CfnOutput(self, f"{construct_id}MskUnderReplicatedPartitionsAlarmArn", value=self.under_replicated_alarm.alarm_arn, export_name=f"{construct_id}-msk-under-replicated-alarm-arn")
        CfnOutput(self, f"{construct_id}MskActiveControllerCountAlarmArn", value=self.active_controller_alarm.alarm_arn, export_name=f"{construct_id}-msk-active-controller-alarm-arn")
        CfnOutput(self, f"{construct_id}MskDiskUsedAlarmArn", value=self.disk_used_alarm.alarm_arn, export_name=f"{construct_id}-msk-disk-used-alarm-arn")
