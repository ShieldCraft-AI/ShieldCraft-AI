...
networking = NetworkingStack(app, "NetworkingStack", config=config.get("networking", {}), vpc_flow_logs_role_arn=vpc_flow_logs_role_arn)
...
lambda_stack = LambdaStack(app, "LambdaStack", vpc=networking.vpc, config=config.get("lambda_", {}), lambda_role_arn=lambda_role_arn)
airbyte_role_arn = Fn.import_value("IamRoleStack-airbyte-role-arn")
airbyte = AirbyteStack(app, "AirbyteStack", vpc=networking.vpc, config=config.get("airbyte", {}), airbyte_role_arn=airbyte_role_arn)
lakeformation = LakeFormationStack(app, "LakeFormationStack", vpc=networking.vpc, config=config.get("lakeformation", {}))
lakeformation_admin_role_arn = Fn.import_value("IamRoleStack-lakeformation-admin-role-arn")
lakeformation = LakeFormationStack(app, "LakeFormationStack", vpc=networking.vpc, config=config.get("lakeformation", {}), lakeformation_admin_role_arn=lakeformation_admin_role_arn)
opensearch_role_arn = Fn.import_value("IamRoleStack-opensearch-role-arn")
opensearch = OpenSearchStack(app, "OpenSearchStack", vpc=networking.vpc, config=config.get("opensearch", {}), opensearch_role_arn=opensearch_role_arn)
...
sagemaker = SageMakerStack(app, "SageMakerStack", vpc=networking.vpc, config=config, sagemaker_role_arn=sagemaker_role_arn)
...
for stack in [networking, msk, s3, glue, iam_roles, lambda_stack, airbyte, lakeformation, opensearch, dataquality, sagemaker, cloud_native_hardening]:
...
compliance_lambda_role_arn = Fn.import_value("IamRoleStack-compliance-lambda-role-arn")
compliance = ComplianceStack(app, "ComplianceStack", config=config.get("compliance", {}), lambda_role_arn=compliance_lambda_role_arn)
