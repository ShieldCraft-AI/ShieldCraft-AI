...
export function login(provider: string): void | Promise<void> {
    ...
    try { if (typeof sessionStorage !== 'undefined') sessionStorage.setItem(PKCE_KEY, verifier); } catch { /* ignore */ }
    ...
    try {
        ...
        for (const redirectUri of candidates) {
            try {
                ...
                try {
                    const verifier = (typeof sessionStorage !== 'undefined') ? sessionStorage.getItem(PKCE_KEY) : null;
