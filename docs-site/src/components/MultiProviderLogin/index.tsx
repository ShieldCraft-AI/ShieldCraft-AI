...
export default function MultiProviderLogin({
    ...
    const availableProviders = React.useMemo(() => {
        ...
        logger.debug('[MPL] getAvailableProviders called', { authKeys: Object.keys(auth || {}) });
        const list = auth.getAvailableProviders();
        if (!Array.isArray(list)) {
            logger.warn('[MultiProviderLogin] No providers available – verify Cognito configuration.');
            return [];
        }
    ...
    const handleProviderLogin = async (providerId: string) => {
        ...
        logger.debug('[MPL] handle start', { providerId, inProgressBefore: inProgress.current.has(providerId) });
        if (inProgress.current.has(providerId)) return;
        inProgress.current.add(providerId);
        logger.debug('[MPL] added to inProgress', providerId);
        try {
            ...
            try {
                ...
                try {
                    logger.debug('[MPL] require.resolve for auth-cognito', require.resolve('../../utils/auth-cognito'));
                } catch { /* ignore */ }
                logger.debug('[MPL] runtimeAuth keys', Object.keys(runtimeAuth || {}), 'providerId', providerId);
                try {
                    logger.debug('[MPL] runtimeAuth.loginWithAmazon === authStatic.loginWithAmazon', runtimeAuth.loginWithAmazon === (authStatic as any).loginWithAmazon);
                } catch { /* ignore */ }
                if (providerId === 'Google') {
                    logger.debug('[MPL] calling runtimeAuth.loginWithGoogle');
                    runtimeAuth.loginWithGoogle();
                } else if (providerId === 'LoginWithAmazon' || providerId === 'Amazon') {
                    logger.debug('[MPL] calling runtimeAuth.loginWithAmazon');
                    runtimeAuth.loginWithAmazon();
                    logger.debug('[MPL] after runtimeAuth.loginWithAmazon');
                } else if (providerId === 'Microsoft' || providerId === 'GitHub') {
                    logger.debug('[MPL] calling runtimeAuth.loginWithProvider', providerId);
            ...
            logger.error(`Login with ${providerId} failed:`, error);
        } finally {
    ...
    const handleProviderEvent = (e: React.SyntheticEvent, providerId: string) => {
        try {
            logger.debug('[MPL] handleProviderEvent', providerId);
            const el = e.currentTarget as HTMLElement | null;
            if (el) {
                const last = el.dataset?.mplLastActivated ? Number(el.dataset.mplLastActivated) : 0;
                logger.debug('[MPL] element lastActivated', last);
                const now = Date.now();
                if (now - last < 300) {
                    logger.debug('[MPL] ignoring duplicate activation', providerId, now - last);
    ...
    return (
        <div className={`${className} provider-menu`} style={containerStyle}>
            {availableProviders.map((provider) => {
                logger.debug('[MPL] render provider', provider && provider.id, provider && provider.name);
