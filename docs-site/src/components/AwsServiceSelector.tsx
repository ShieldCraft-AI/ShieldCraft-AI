...
export type AwsServiceSelectorProps = {
    ...
    selected?: string;
    /** callback when user selects a service */
...
export default function AwsServiceSelector({
    ...
    useEffect(() => {
        // initialize refs array to length of services
        if (!buttonsRef.current || buttonsRef.current.length !== FLAT_SERVICES.length) {
            buttonsRef.current = new Array(FLAT_SERVICES.length).fill(null);
        }
    }, []);
            ...
            <div className={styles.selector}>
                        ...
                        return (
                            ...
                            >
                                <span className={styles.iconWrap} aria-hidden="true">
                                    {/* use service-provided icon path (absolute under docs-site/static/aws-icons/) */}
                                    <img
                                        src={svc.icon}
                                        alt=""
                                        width={20}
                ...
                <div className={styles.right}>
                    <div
                        id={`svc-${selected}`}
                        role="tabpanel"
                        aria-labelledby={`svc-tab-${selected}`}
