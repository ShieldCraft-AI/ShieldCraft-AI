...
def test_safe_mkdir_warning_and_exit_code(caplog, tmp_path, monkeypatch):
    ...
    warnings = [
        ...
        if "Unable to create directory" in record.message
        and "drift_telemetry" in record.message
    ]
    assert warnings, "Expected warning from _safe_mkdir when mkdir fails"
