...
def test_stack_telemetry_log_shape(caplog, tmp_path, monkeypatch):
    ...
    for record in caplog.records:
        if not record.message.startswith("[telemetry] "):
            continue
        telemetry_records.append(json.loads(record.message.split(" ", 1)[1]))
    stack_entry = next(
        item for item in telemetry_records if item["telemetry_type"] == "drift_stack"
    )
    assert stack_entry["stack"] == "TestStack"
    assert stack_entry["comparison_status"] in {"acknowledged", "new_drift", "no_drift"}
    summary_entry = next(
        item for item in telemetry_records if item["telemetry_type"] == "drift_summary"
    )
    assert summary_entry["stacks_scanned"] == 1
    assert summary_entry["ci_forced"] is False


def test_summary_marks_ci_runs(caplog, tmp_path, monkeypatch):
    caplog.set_level(logging.INFO, logger="drift_remediation")
    mod = _setup_module(monkeypatch, tmp_path)
    artifacts_dir = tmp_path / "artifacts" / "drift"
    ...
    assert summary_entry["ci_forced"] is True


def test_metadata_file_written_when_flag_set(tmp_path, monkeypatch):
    mod = _setup_module(monkeypatch, tmp_path)
    artifacts_dir = tmp_path / "artifacts" / "drift"
    ...
    assert "drift_new_detected" in payload
    assert payload["ci_forced"] is True


def test_metadata_file_not_written_without_flag(tmp_path, monkeypatch):
    mod = _setup_module(monkeypatch, tmp_path)
    artifacts_dir = tmp_path / "artifacts" / "drift"
