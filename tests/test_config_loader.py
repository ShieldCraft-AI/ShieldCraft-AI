...
def test_secret_resolution_extension_point():
    ...
    class SecretBackend:
        def load(self, env):
            return {'app': {'env': 'dev', 'secret': 'aws-vault:my-secret'} , 's3': {}, 'glue': {}, 'msk': {}}
...
import pytest
from infra.utils.config_loader import ConfigLoader, ConfigSchema
...
def test_secret_resolution_nonmatch():
    config = {'app': {'env': 'dev', 'notsecret': 'plain'}, 's3': {}, 'glue': {}, 'msk': {}}
...
def test_export_redacts_nested_secrets():
    config = {'app': {'env': 'dev', 'secrets': ['aws-vault:foo', 'bar']}, 's3': {}, 'glue': {}, 'msk': {}}
    loader = ConfigLoader(env='dev', backend=DummyBackend(config))
...
import yaml
import pytest
from infra.utils.config_loader import ConfigLoader, get_config_loader, ConfigSchema
from infra.utils.config_backends import LocalYamlBackend, S3Backend, SSMBackend
...
def test_secret_resolution():
    config = {'app': {'env': 'dev', 'secret': 'aws-vault:my-secret'}, 's3': {}, 'glue': {}, 'msk': {}}
    loader = ConfigLoader(env='dev', backend=DummyS3Backend(config))
    val = loader.get('app.secret')
    assert val.startswith('[REDACTED:')
