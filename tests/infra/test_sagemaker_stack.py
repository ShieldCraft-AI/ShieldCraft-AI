...
def sagemaker_config():
    return {
        "sagemaker": {
            ...
            "image_uri": "123456789012.dkr.ecr.us-east-1.amazonaws.com/shieldcraft:latest",
            ...
            "cost_alarm_sns_topic_arn": "arn:aws:sns:us-east-1:123456789012:CostTopic"
        },
        "app": {"env": "test", "owner": "alice", "data_classification": "internal", "component": "ml-inference"}
    }
...
def test_sagemaker_stack_synthesizes(sagemaker_config):
    ...
    for key in ["model", "endpoint_config", "endpoint", "endpoint_status_failed_alarm", "invocation_4xx_errors_alarm", "model_latency_alarm"]:
...
def test_sagemaker_stack_outputs_are_correct(sagemaker_config):
    ...
    assert get_val(outputs["shieldcraftmodelModelName"]) == sagemaker_config["sagemaker"]["model_name"]
    assert get_val(outputs["shieldcraftmodelEndpointConfigName"]) == "shieldcraft-model-config"
    assert get_val(outputs["shieldcraftmodelEndpointName"]) == sagemaker_config["sagemaker"]["endpoint_name"]
...
def test_sagemaker_stack_shared_resources_keys(sagemaker_config):
    ...
    expected = {
        ...
        "endpoint_status_failed_alarm", "invocation_4xx_errors_alarm", "model_latency_alarm",
        "cpu_utilization_alarm", "memory_utilization_alarm"
    }
...
def test_sagemaker_stack_advanced_tags(sagemaker_config):
    ...
    assert any(tag.get("Key") == "Component" and tag.get("Value") == "ml-inference" for tag in tags)
...
def test_sagemaker_stack_alarm_properties(sagemaker_config):
    ...
    alarm_props = [r["Properties"] for r in resources.values() if r["Type"] in alarm_types]
...
def test_sagemaker_stack_model_name_special_chars():
    ...
    assert any("shieldcraft_model2025" in k or "shieldcraftmodel2025" in k for k in outputs)
...
def test_sagemaker_stack_alarm_arns_are_strings(sagemaker_config):
    ...
    for alarm in ["endpoint_status_failed_alarm", "invocation_4xx_errors_alarm", "model_latency_alarm"]:
        ...
        assert isinstance(arn, str) or hasattr(arn, "to_string"), f"Alarm ARN for {alarm} is not a string or token"
